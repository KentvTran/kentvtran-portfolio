...
{:else}
	<!-- unchanged mobile markup -->
	<div class="flex items-center">
		<PrismicLink
			class="group relative flex items-center justify-center overflow-hidden rounded-full text-xl font-medium transition-all duration-300 ease-in-out px-6 py-3"
			{field}
			on:click={onLinkClick}
			aria-current={isActive ? 'page' : undefined}
		>
