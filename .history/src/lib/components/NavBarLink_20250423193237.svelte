...
For serve_static() (safer path handling):
...
void serve_static(int client_fd, const char* path) {
    ...
    char *full_path = NULL;
    if (asprintf(&full_path, "%s/%s", STATIC_DIR, path) == -1) {
        reply_error(client_fd, HTTP_500, "Memory error");
        return;
    }

    FILE* file = fopen(full_path, "rb");
    free(full_path); // Free early to avoid leaks
    // ... rest of function ...
}
Bottom Line: Your code is readable and works for small-scale use. strdup()/dynamic allocation would make it more resilient to edge cases but adds memory management complexity. Choose based on your priorities!