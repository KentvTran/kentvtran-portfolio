<script lang="ts">
	...
	function updateCursor(position: { left: number; width: number; opacity: number }) {
		gsap.to(cursorEl, {
			left: position.left,
		...
		if (position.opacity === 1) {
			activePosition = { left: position.left, width: position.width };
		}
	}
		...
		<div class="flex flex-col items-center space-y-6">
			...
			<div class="relative z-50 hidden md:block">
				...
				>
					...
					{/each}
					<div
						class="absolute z-0 h-7 rounded-full bg-gray-200 opacity-0"
						bind:this={cursorEl}
					/>
				</div>
			</div>
