<script lang="ts">
	...
	// Modified resetCursor function to keep highlighting the active page
	function resetCursor() {
		// Find the active link
		const activeLink = navContainer?.querySelector('[aria-current="page"]') as HTMLElement | null;
		
		if (activeLink) {
			// If there's an active link, position the cursor on it
			...
			updateCursorPosition(
				relativeLeft,
				rect.width,
				1 // Keep opacity at 1 for active link
			);
		} else {
			// If no active link, hide the cursor
			updateCursorPosition(activePosition.left, activePosition.width, 0);
		}
	}

	// Function to handle active link highlighting on page load and when page changes
	$: if ($page && navContainer) {
