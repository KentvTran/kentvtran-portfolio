...
function handleMouseEnter() {
  if (type === "desktop" && updateCursorPosition && linkWrapper) {
    ...
    // Slightly expand the width for a more generous hover area
    const expandedWidth = rect.width + 10
    const adjustedLeft = relativeLeft - 5 // Adjust left to center the expanded width

    updateCursorPosition(adjustedLeft, expandedWidth)
  }
}
