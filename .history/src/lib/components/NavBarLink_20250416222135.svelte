<script lang="ts">
  ...
  $: {
    if ($page) {
      ...
      if (newActiveIndex !== -1) {
        ...
        if (motion && highlightElement && navItems[activeIndex]) {
  ...
  function moveHighlightToItem(index) {
    ...
    // Use the motion library to animate the highlight
    motion.to({
      el: highlightElement,
      translateX: rect.left - containerRect.left,
      width: rect.width,
      duration: 0.6,
      easing: 'easeOutExpo'
    });
